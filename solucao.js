...
function exercicio04(disponiveisDestaques) {
  disponiveisDestaques = listaProdutos.filter(item => item.disponivel.includes('sim') && item.emDestaque.includes('sim')).reduce((prevent, current) => prevent + current.qtdEstoque, 0)
...
function exercicio11() {
  ...
  console.log('Quantidade de itens no estoque por derpatamento', deps)
}
exercicio11()
...
function exercicio12() {
  const inventarioDepto = listaProdutos.reduce((prevent, current) => {
    let inventario = current.qtdEstoque * current.preco  * 100
    prevent[current.departamento.nomeDepto] = (prevent[current.departamento.nomeDepto] || 0) + inventario
    return prevent
    
  }, {})
  
  for (const depto in inventarioDepto) {
    inventarioDepto[depto] = inventarioDepto[depto] / 100
  }
    console.log('Valor total de inventário por departamento é de R$', inventarioDepto)
}
exercicio12()
...
function exercicio13() {
  const ticketDepto = listaProdutos.reduce((prevent, current) => {
    let ticketValor = (current.qtdEstoque * current.preco) / listaProdutos.length
    prevent[current.departamento.nomeDepto] = (prevent[current.departamento.nomeDepto] || 0) + ticketValor
    return prevent

  }, {})

  console.log('O ticket médio por departamento é: R$', ticketDepto)
}
exercicio13()
...
function exercicio14() {
  const deptos = listaProdutos.reduce((prevent, current) => {
    prevent[current.departamento.nomeDepto] = (prevent[current.departamento.nomeDepto] || 0) + ((current.qtdEstoque * current.preco))
    return prevent

  }, {})
 
  const deptoValioso = Object.keys(deptos).reduce((prevent, current) => {
    if (!prevent) {
      return current
    }

    if (deptos[current] > deptos[prevent]) {
      return current
    }
  ...
  console.log('O departamento mais valioso é o de', deptoValioso, 'com inventario no valor de R$', deptos[deptoValioso].toFixed(2))
}
...
function exercicio15() {
  ...
  console.log('O departamento menos valioso é o de', deptoValioso, 'com inventario no valor de R$', deptos[deptoValioso].toFixed(2))
}
exercicio15()